    ...
    def recpop(self, train=None, test=None, k=8, nitem=10):
        ...
        for user in train.keys():
            ...
            for item, ratings in rank.items():
                ret += math.log(1 + item_popularity[item])
                n += 1
        return rank.items(), ret / (n * 1.0)


def testRecommend():
    ubcf = UserBasedCF('C:/Users/HP/Desktop/u.data')
    ubcf.readData()
...
def testUserBasedCF():
    cf = UserBasedCF('C:/Users/HP/Desktop/u1.data')
    ...
    for k in [5, 10, 20, 40, 80, 160]:
        ...
        print("%.13s%3d%19.3f%%%19.3f%%%19.3f%%%20.3f" % (
            "UserCF       ", k, recall * 100, precision * 100, coverage * 100, popularity))
...
def testUserBasedCF_IIF():
    ...
    for k in [80]:
        ...
        popularity = cf.popularity(k=k)
        print("%.13s%3d%19.3f%%%19.3f%%%19.3f%%%20.3f" % (
            "UserCF_IIF   ", k, recall * 100, precision * 100, coverage * 100, popularity))
...
def RecommandPopularity():
    ...
    for k in [40]:
        ...
        print("%.13s%13.3f" % ("UserCF_IIF   ", popularity), "%16s" % (recommandlist))
